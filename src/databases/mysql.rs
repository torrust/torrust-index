...
impl Database for MysqlDatabase {
    ...
    async fn insert_torrent_and_get_id(&self, torrent: &Torrent, uploader_id: i64, category_id: i64, title: &str, description: &str) -> Result<i64, DatabaseError> {
        ...
        // add torrent
        let torrent_id = query("INSERT INTO torrust_torrents (uploader_id, category_id, info_hash, size, piece_length, pieces, root_hash, date_uploaded) VALUES (?, ?, ?, ?, ?, ?, ?, UTC_TIMESTAMP())")
            .bind(uploader_id)
            .bind(category_id)
            .bind(info_hash)
