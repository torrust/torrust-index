...
impl Database for Sqlite {
    ...
    async fn ban_user_and_revoke_tokens(
        ...
        let ban_result = query("INSERT INTO torrust_user_bans (user_id, reason, date_expiry) VALUES ($1, $2, $3)")
            .bind(user_id)
            .bind(reason)
            .bind(date_expiry_string)
            .execute(&mut *tx)
    ...
    async fn is_user_banned(&self, user_id: i64) -> Result<bool, database::Error> {
        query_as::<_, (i64,)>(
            "SELECT COUNT(*) FROM torrust_user_bans WHERE user_id = ? AND date_expiry > strftime('%Y-%m-%d %H:%M:%S', 'now')",
        )
        .bind(user_id)
        .fetch_one(&self.pool)
        .await
