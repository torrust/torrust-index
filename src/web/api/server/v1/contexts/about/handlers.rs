//! API handlers for the the [`about`](crate::web::api::server::v1::contexts::about) API
//! context.
use std::sync::Arc;

use axum::extract::State;
use axum::http::{StatusCode, header};
use axum::response::{IntoResponse, Response};

use crate::common::AppData;
use crate::web::api::server::v1::extractors::require_permission::{GetAboutPage, GetLicensePage, RequirePermission};

#[allow(clippy::unused_async)]
pub async fn about_page_handler(
    State(app_data): State<Arc<AppData>>,
    RequirePermission(_actor, _): RequirePermission<GetAboutPage>,
) -> Response {
    match app_data.about_service.get_about_page() {
        Ok(html) => (StatusCode::OK, [(header::CONTENT_TYPE, "text/html; charset=utf-8")], html).into_response(),
        Err(error) => error.into_response(),
    }
}

#[allow(clippy::unused_async)]
pub async fn license_page_handler(
    State(app_data): State<Arc<AppData>>,
    RequirePermission(_actor, _): RequirePermission<GetLicensePage>,
) -> Response {
    match app_data.about_service.get_license_page() {
        Ok(html) => (StatusCode::OK, [(header::CONTENT_TYPE, "text/html; charset=utf-8")], html).into_response(),
        Err(error) => error.into_response(),
    }
}
