...
use crate::web::api::server::v1::extractors::require_permission::{GetImageByUrl, RequirePermission};

/// Get the remote image. It uses the cached image if available.
#[allow(clippy::unused_async)]
pub async fn get_proxy_image_handler(
    State(app_data): State<Arc<AppData>>,
    RequirePermission(actor, _): RequirePermission<GetImageByUrl>,
    ...
    match app_data.proxy_service.get_image_by_url(&image_url, actor.user_id()).await {
        Ok(image_bytes) => {
