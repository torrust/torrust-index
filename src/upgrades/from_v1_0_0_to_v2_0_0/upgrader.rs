...
/// Run the v1.0.0 to v2.0.0 upgrade with the current timestamp.
///
/// # Errors
///
/// Returns an error if any database setup, migration, truncation, or transfer
/// step fails.
pub async fn run(args: Arguments) -> Result<(), UpgradeError> {
    let now = datetime_iso_8601();
    upgrade(&args, &now).await
}

/// Upgrade data from v1.0.0 to v2.0.0 using a caller-supplied import timestamp.
///
/// # Errors
///
/// Returns an error if any database setup, migration, truncation, or transfer
/// step fails.
pub async fn upgrade(args: &Arguments, date_imported: &str) -> Result<(), UpgradeError> {
    ...
    info!("upgrading data from version v1.0.0 to v2.0.0");
