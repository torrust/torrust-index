...
- [ADR-T-011: Minimum Supported Rust Version Policy](adr/011-msrv-policy.md) — Compute `rust-version` as the newest stable Rust released at least one year before the day the pin is computed, recomputed at every release and in every maintenance pass; the current pin is read from the workspace manifest's `rust-version` field, and each raise is recorded in the changelog naming the policy.
