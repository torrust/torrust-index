...
fn option_some_maps_to_value() {
    assert_eq!(option_to_metric(Some(42.5)).to_bits(), 42.5f64.to_bits());
}
...
fn boolean_maps_to_zero_or_one() {
    ...
    let drift = samples
        ...
        .unwrap();
    assert_eq!(drift.value.to_bits(), 0.0f64.to_bits());
    ...
    let drift = samples
        ...
        .find(|s| s.name == "assayer_feature_stable_outcome_drift")
        .unwrap();
    assert_eq!(drift.value.to_bits(), 1.0f64.to_bits());
}
...
fn convergence_stage_maps_to_integer() {
    ...
    assert_eq!(stage.value.to_bits(), 5.0f64.to_bits());
}
...
fn platt_state_maps_to_integer() {
    ...
    for (state, expected) in [
        (PlattConvergenceState::Initial, 0.0f64),
        (PlattConvergenceState::FirstFit, 1.0f64),
        (PlattConvergenceState::Converging, 2.0f64),
        (PlattConvergenceState::Converged, 3.0f64),
    ] {
        summary.platt_state = state;
        let samples = health_summary_to_samples(&summary);
        let s = samples.iter().find(|s| s.name == "assayer_platt_state").unwrap();
        assert_eq!(s.value.to_bits(), expected.to_bits(), "PlattConvergenceState::{state:?}");
    }
}
...
fn health_summary_to_samples_has_every_field() {
    ...
    assert_eq!(find("assayer_assessments_total").value.to_bits(), 10_000.0f64.to_bits());
    assert_eq!(find("assayer_labels_total").value.to_bits(), 500.0f64.to_bits());
    assert_eq!(find("assayer_labels_eligible_total").value.to_bits(), 450.0f64.to_bits());
    assert_eq!(
        find("assayer_signal_shape_mismatches_total").value.to_bits(),
        4.0f64.to_bits()
    );
    assert_eq!(find("assayer_unknown_signals_total").value.to_bits(), 6.0f64.to_bits());
    assert_eq!(
        find("assayer_zero_sentinel_assessments_total").value.to_bits(),
        7.0f64.to_bits()
    );
    assert_eq!(find("assayer_drift_resets_total").value.to_bits(), 4.0f64.to_bits());
    assert_eq!(find("assayer_convergence_stage").value.to_bits(), 3.0f64.to_bits());
    assert_eq!(find("assayer_platt_delta_cal").value.to_bits(), 0.05f64.to_bits());
    assert_eq!(find("assayer_blend_weight_mean").value.to_bits(), 0.15f64.to_bits());
    assert_eq!(find("assayer_feature_stable_outcome_drift").value.to_bits(), 1.0f64.to_bits());
    assert_eq!(find("assayer_quantile_error_concentration").value.to_bits(), 1.8f64.to_bits());
    assert_eq!(find("assayer_auc_aggregate").value.to_bits(), 0.82f64.to_bits());
    assert_eq!(find("assayer_positive_class_prior_global").value.to_bits(), 0.37f64.to_bits());
    assert_eq!(
        find("assayer_positive_class_prior_eligible").value.to_bits(),
        0.61f64.to_bits()
    );
}
...
fn full_report_standardisation_snapshot_version() {
    ...
    assert_eq!(version.value.to_bits(), 47.0f64.to_bits());
}
...
fn positive_class_priors_reach_summary_samples() {
    ...
    assert_eq!(global.value.to_bits(), 0.29f64.to_bits());
    assert_eq!(eligible.value.to_bits(), 0.73f64.to_bits());
}
...
fn drift_reset_counter_reaches_summary_samples() {
    ...
    assert_eq!(resets.value.to_bits(), 6.0f64.to_bits());
}
...
fn full_report_signal_cache_metrics_carry_live_statistics() {
    ...
    assert_eq!(find("assayer_signal_cache_evictions_total").value.to_bits(), 4.0f64.to_bits());
    assert_eq!(find("assayer_signal_cache_hit_rate").value.to_bits(), 0.75f64.to_bits());
    assert_eq!(find("assayer_signal_cache_utilisation").value.to_bits(), 0.25f64.to_bits());
}
...
fn full_report_sentinel_coverage_carries_reporting_fraction() {
    ...
    assert_eq!(empty_coverage.value.to_bits(), 0.0f64.to_bits());
}
...
fn full_report_concordance_calibrations() {
    ...
    let conc = samples
        ...
        .unwrap();
    assert_eq!(conc.value.to_bits(), 7.0f64.to_bits());
}
...
fn cascade_terminus_emitted_from_summary_only() {
    ...
    let ct = summary_samples
        ...
        .unwrap();
    assert_eq!(ct.value.to_bits(), 1.0f64.to_bits());
...
fn health_events_dropped_emitted_from_summary_only() {
    ...
    let dropped = summary_samples
        ...
        .unwrap();
    assert_eq!(dropped.value.to_bits(), 42.0f64.to_bits());
