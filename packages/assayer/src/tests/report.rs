...
fn axis_score_set_all_zeroes() {
    ...
    assert_eq!(set.novelty.max_z.to_bits(), 0.0f64.to_bits());
    assert_eq!(set.novelty.mean_z.to_bits(), 0.0f64.to_bits());
    assert_eq!(set.novelty.cusum.to_bits(), 0.0f64.to_bits());
    assert_eq!(set.novelty.baseline_mean.to_bits(), 0.0f64.to_bits());
    assert_eq!(set.novelty.baseline_variance.to_bits(), 0.0f64.to_bits());
    assert_eq!(set.novelty.clip_pressure.to_bits(), 0.0f64.to_bits());

    assert_eq!(set.displacement.max_z.to_bits(), 0.0f64.to_bits());
    assert_eq!(set.surprise.max_z.to_bits(), 0.0f64.to_bits());
    assert_eq!(set.coherence.max_z.to_bits(), 0.0f64.to_bits());
}
...
fn axis_score_set_independent() {
    ...
    assert_eq!(
        set.displacement.max_z.to_bits(),
        0.0f64.to_bits(),
        "displacement should be independent"
    );
    assert_eq!(
        set.surprise.max_z.to_bits(),
        0.0f64.to_bits(),
        "surprise should be independent"
    );
    assert_eq!(
        set.coherence.max_z.to_bits(),
        0.0f64.to_bits(),
        "coherence should be independent"
    );

    // Set another axis
    set.displacement.mean_z = 3.0;
    assert_eq!(
        set.novelty.mean_z.to_bits(),
        0.0f64.to_bits(),
        "novelty.mean_z should be independent"
    );
}
