...
fn force_checkpoint(assayer: &Assayer, failure_message: &str) {
    let (ack_tx, ack_rx) = crossbeam_channel::bounded(1);
    assayer
        .command_tx()
        .send(crate::owner::commands::ModelOwnerCommand::Checkpoint(
            crate::owner::commands::CheckpointRequest {
                completion: Some(ack_tx),
            },
        ))
        .unwrap();
    ack_rx.recv_timeout(ACK_DEADLINE).unwrap().expect(failure_message);
}
...
fn checkpoint_restores_runtime_registries_with_rebind_semantics() {
    ...
    {
        let assayer = test_assayer_with_persistence(dir.path(), "registry-capture");
        assayer.register_sentinel(sentinel_reg(sentinel)).unwrap();
        assayer.register_outcome_axis(spatial_axis_reg(axis)).unwrap();
        let version = wait_for_version(assayer.shared(), 3, crate::testing::STATE_DEADLINE);
    ...
    let restored = test_assayer_with_persistence(dir.path(), "registry-restore");
...
fn checkpoint_restores_identity_state_on_registration() {
    ...
    {
        let assayer = test_assayer_with_persistence(dir.path(), "identity-capture");
        assayer.register_identity_dimension(registration.clone()).unwrap();
        wait_for_version(assayer.shared(), 2, crate::testing::STATE_DEADLINE);
        {
        ...
        force_checkpoint(&assayer, "first checkpoint should succeed");
        wait_for_version(assayer.shared(), 3, crate::testing::STATE_DEADLINE);
        ...
        force_checkpoint(&assayer, "second checkpoint should succeed");
    }

    let restored = test_assayer_with_persistence(dir.path(), "identity-restore");
