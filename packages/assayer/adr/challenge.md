...
This record owns the challenge estimate: the conjugate model behind it, the boundary that keeps it host-owned, the sufficiency floor its health report reads, the final arrangement of its public surfaces, and the fact that it shipped before anything consumed it. It realises the challenge-model choice (`dec:numerics:challenge-model`) and the Companion boundary choice (`dec:contracts:companion-boundary`).

It carries a finished model, a final host-facing arrangement, and no connection from the Companion into Core. The absence of that connection is part of the arrangement rather than a gap in it.
...
**Decision (The two existing surfaces are the final Companion arrangement)** · `dec:challenge:arrangement-final`

The Companion's whole surface is the published replacement trait (`req:companion:replacement-trait`) and the clocked standalone health report with the fields the specification assigns it (`tab:companion:health`). No metrics mapper joins them. The challenge metric names remain host-emitted under the Assayer prefix.

Host ownership fixes that division. The host holds the Companion state, chooses the provider, and reads the report, while the Companion reaches no Core structure (`dec:challenge:host-ownership`) and (`dec:contracts:companion-boundary`). The host is therefore already the consumer with both the values and the point of emission; moving their mapping into the package would either require a path back into host-owned state or ask the host to hand the same values back for an identity conversion.

A mapper under a Companion prefix is rejected because it would add a third surface with no consumer and a second metrics prefix that must be versioned beside the Assayer catalogue. A mapper under the Assayer prefix is rejected because the existing names already give the host that vocabulary, while a package mapper cannot source the host-owned state without crossing the boundary the arrangement preserves. The legacy three-part description expressed an intended arrangement rather than a compatibility promise: no consumer relies on the mapper that never existed, while replacement and direct health observation are both served by the two public surfaces that do.
