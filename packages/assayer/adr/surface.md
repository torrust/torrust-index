...
**Corollary (Test support preserves host opacity)** · `cor:surface:test-support-opacity`

Test-only state projections admitted by (`dec:harness:probe-contract`) remain inside the gated testing reach of (`sec:assayer:testing-architecture-reach`), so a sealed host-facing build cannot name them. This follows from the structural opacity of (`dec:surface:guidance-opacity`): a test need creates no additional host surface.

The specialised arrangements retained by (`dec:harness:specialised-side-harnesses`) stay behind the same boundary. A test may rely on their narrow seams where its subject requires them; neither their projections nor their ceremony may be promoted into the host-facing contract.
