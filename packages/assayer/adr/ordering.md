...
**Corollary (Playback batches share cost, not a view)** · `cor:ordering:tape-batches`

A playback runner may batch rows to bound execution cost, but the batch inherits only the shared timestamp of (`dec:ordering:batch-timestamp`); it is neither a consistency boundary nor a promise that its rows share a published view. Mid-batch identity movement makes that distinction observable (`entry:assayer:mid-batch-identity-movement`), so a test comparing settled views across rows takes the required barrier between them. Playback progress identifies the last completed row for liveness diagnosis and carries no latency promise (`cav:harness:progress-not-latency`).
