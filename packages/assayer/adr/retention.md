...
**Caveat (Probes stop at the retention boundary)** · `cav:retention:probe-boundary`

The precision matrix remains excluded under (`dec:retention:precision-excluded`), and the probe contract of (`dec:harness:probe-contract`) cannot turn that exclusion into a test-support projection. A promise about deliberately excluded state is witnessed directly at crate level, where the working state can be examined without widening either the published snapshot or the host surface.

The requirement that every state projection use the probe contract therefore ranges over every projection the test-support contract permits. A test may rely on the probe discipline for those projections; it must not request a probe for excluded state and thereby make retention less strict.
