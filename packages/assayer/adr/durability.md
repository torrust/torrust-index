...
**Corollary (A persistence witness forks from isolated durable state)** · `cor:durability:harness-fork`

A persistence witness under the playback contract of (`dec:harness:declarative-playback`) quiesces the instance, takes the checkpoint of (`dec:durability:checkpoint-journal`), and copies the checkpoint and journal into isolated storage. It rebuilds from that copy under the same injected clock, in the persistent domain fixed by (`dec:clock:two-domains`), runs the uninterrupted and restored arms through the same suffix, and compares one normalised projection of their durable state. Quiescence fixes the prefix, isolation prevents either arm from consuming or extending the other's journal, and the common clock leaves restore to apply the elapsed decay exactly once as decided at (`dec:durability:decay-once`).

No further persistence choice follows. A test may rely on equality of the normalised projections after the common suffix; it may not compare live files, share storage between the arms, or advance their clocks differently and call the resulting difference a replay failure.
