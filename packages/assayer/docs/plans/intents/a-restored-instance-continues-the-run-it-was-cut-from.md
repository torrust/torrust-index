...
Restored construction attaches the recovered Ledger, concordance and owner state, opens the journal after the restored sequence, and sends replay entries through the ordinary owner pipeline (´dec:durability:checkpoint-journal´). It reconstructs runtime Sentinel slots and outcome-axis membership from the restored working copy; the first matching registration consumes a rebind marker and extends no model, while a report can reach the Sentinel before that declaration is repeated. Recovered identity payloads wait for the corresponding host registration because the encode closure is process-local; that registration attaches the closure, reconstructs the owner-held graph, and publishes the stored competitive cells and cell outcomes without extending the restored model. Construction still resets the assessment counter, blend tracker, several degradation and label-integrity counters, health-event counts and publication numbering, so those remaining fresh values keep the full promise observably false.
...
The runtime-rebinding mechanism is present across construction and lifecycle. Sentinel and outcome-axis registries are reconstructed from the working copy, and the first matching registration rebinds without adding duplicate feature positions. Each identity payload remains pending until the host repeats that dimension's registration, whose encode closure reconnects the dedicated owner while the payload seeds its restored graph, competitive set and cell outcomes; no model extension occurs. The remaining work for this entry is the declarative registration tape and state projection needed by the full witness.
