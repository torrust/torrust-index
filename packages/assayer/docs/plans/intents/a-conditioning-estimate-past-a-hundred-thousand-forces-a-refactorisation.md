...
This plan states what a test keeping (´claim:bayes:a-conditioning-estimate-past-a-hundred-thousand-forces-a-refactorisation´) has to distinguish: growth of the diagonal lower bound past its baseline brings a covariance rebuild forward before the label counter does, the witness observes that transition in the $10^5$ regime without making that absolute reading its cause, and the adopted rebuild leaves the precision and covariance pair within its synchronisation ceiling.
...
The promise has a relative trigger and a postcondition. Before the configured recomputation interval expires, the diagonal lower bound grows past its recorded baseline by the configured factor while the witness operates above $10^5$; that relative crossing causes one Cholesky refactorisation, the absolute figure fixes the scenario's regime rather than its cause, and the refactorisation leaves $\lVert B\tilde{\Sigma}-I\rVert_F$ near zero.
...
The trigger contract is growth of the diagonal lower bound past its recorded baseline by the configured factor: conditioning at an absolute ratio of a million million calls for nothing when it has not grown relative to that baseline (´claim:bayes:no-absolute-conditioning-reading-calls-for-a-rebuild´), kept by (´test:crate:conditioning-alone-never-calls-for-a-rebuild´). The witness therefore brackets the relative crossing before the counter expires, holds the other arms still, and treats $10^5$ as its fixture regime rather than its cause. Carrying this settled contract into the witness is approximately fifteen assertion and diagnostic lines and depends on no sibling lane.
...
The current algorithm states that an absolute conditioning threshold is permanently true after a rebuild because rebuilding the covariance does not change the precision matrix (´alg:gaussian:condition-adaptive-recompute´). The public-surface evidence records the resulting refactorisation on every label (´rep:assayer:synchronisation-drift-study´). The settled witness therefore brackets growth past the recorded baseline, excludes the competing arms, and observes the next-label reset control; merely arranging for $\rho(B)>10^5$ and observing a rebuild supplies no cause.
...
The implementation report identifies growth of the diagonal lower bound past its recorded baseline as the trigger and cites the matching specification and record. The intent text and witness treat $10^5$ only as a scenario precondition, and the report includes the reset control that demonstrates why the unchanged absolute reading does not fire again.
