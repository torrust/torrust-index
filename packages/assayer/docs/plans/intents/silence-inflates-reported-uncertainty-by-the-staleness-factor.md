...
The promise therefore assigns `F` to the variance and asserts `stale.sigma_eff / fresh.sigma_eff = sqrt(F)` and `stale.uncertainty / fresh.uncertainty = sqrt(F)` in a fixture whose probability and calibration scale are unchanged; equivalently, either squared ratio is `F` within the harness default tolerance.
...
- Assertion: compare each public standard-deviation ratio with `sqrt(F)` and each squared ratio with `F` via `assert_near`.
...
**DONE.** The promise binds `F` to variance, `sqrt(F)` to both public standard-deviation ratios, and `F` to their squared ratios. The catalogue claim and future witness use that dimensional relation, with no harness or sibling-lane dependency.
...
Add the integration test, its module index row, and its generated test-catalog row now that the unit is aligned. The change is approximately 55–80 lines and uses only existing harness pieces.
...
`F` is the covariance and variance multiplier, so `sigma_eff` and `uncertainty` grow by `sqrt(F)` at fixed probability and calibration scale, and their squares grow by `F`. Assigning `F` to the public standard deviation would require a squared variance correction or a public-field semantic change and would conflict with the temporal and risk definitions.
...
The implementation report identifies the settled variance unit contract, the aligned intent text, and the integration witness that cites the promise.
