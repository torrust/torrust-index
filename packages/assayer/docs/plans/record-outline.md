...
| ``harness`` | The shared testing contract: one scenario, declared barriers and playback, bounded probes, independent oracles, guarded fixtures, and explicit side-harness limits. | (´dec:assayer:test-documentation-policy´), (´plan:assayer:testing-harness-concept´) | 10 | 1 |
...
The testing-only entry is placed after ``clock`` and before ``vector`` because the operational section is the closest home for a contract governing time, queues, playback, and state observation, while the ruling above keeps its source explicitly outside the layer derivation. Its projected inventory is fixed by (´tab:assayer:testing-harness-record-audit-environments´).
...
| ``cav`` | ``cav:harness:convergence-concentration`` | the shared contract does not decide whether the crowded convergence test module splits |
| ``cav`` | ``cav:harness:incremental-adoption`` | the single-scenario claim becomes true only when the last parallel model and wait vocabulary retire |
