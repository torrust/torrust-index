...
The caveat audit finds **nothing that cold standardisation amends or discharges in the challenge or health records**. The Companion arrangement caveat has since retired under the final arrangement (`dec:challenge:arrangement-final`). The remaining verified challenge mints are (`cav:challenge:unwired`) (`cav:challenge:sufficiency-threshold-open`) (`cav:challenge:thin-evidence`), and the verified health mints are (`cav:health:identity-maturity-arithmetic`) (`cav:health:dead-convergence-thresholds`) (`cav:health:tracker-event-defect`). None states or warrants cold standardisation, and treating one as overtaken would be a false derivation. The vector record's late-Sentinel caveat remains live and unchanged (`cav:vector:late-standardisation`).
