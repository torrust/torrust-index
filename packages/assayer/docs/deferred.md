...
**The migration is complete, and the register is open again.** One new deferral stands below and every inherited entry has been implemented or retired at the record that owns it under (`sec:assayer:defer-retirements`). The linted list remains a stable set of heads that other documents cite, each resolving into the record where the deferral is stated in full. Nothing here needs re-verifying against the code, because nothing here claims anything about the code — the records do.

An open register is not a second decision home. The file stays because its heads are cited from across the corpus, while the substance of each deferral remains in its owning record.
...
The testing-harness convergence layout is open; every earlier entry this section held has completed and stands under (`sec:assayer:defer-retirements`).

**Entry (Testing-harness convergence layout)** · `entry:assayer:defer-harness-convergence-layout`

(`cav:harness:convergence-concentration`)
