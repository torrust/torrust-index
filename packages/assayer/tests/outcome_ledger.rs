...
//! | [`outcome_predictions_do_not_perturb_reckoning_when_core_history_matches`] | ledger | Outcome-axis predictions stay outside the derivation even once they are live. One assessment carrying a finite prediction and uncertainty band is cloned, only that output payload is cleared, and both forms derive bit-identical risk bases and tag profiles. Predictions are an output the host may consult, never an input that feeds back into the risk the engine assigns. |
...
/// live. One assessment carrying a finite prediction and uncertainty band is
/// cloned, only that output payload is cleared, and both forms derive
/// bit-identical risk bases and tag profiles. Predictions are an output the
/// host may consult, never an input that feeds back into the risk the engine
/// assigns.
///
...
fn outcome_predictions_do_not_perturb_reckoning_when_core_history_matches() {
    ...
    // must not read those prediction outputs. Train one world until its
    // axis model emits a live prediction, then derive once from that
    // assessment and once from a clone with only the prediction payload
    // removed. Holding the input risk basis fixed takes model-training
    // reduction order out of the comparison entirely.
    ...
    let assessment = with_axis.core_assess(with_axis.request("default", "alice"));
    let with_prediction = with_axis.derive(&assessment, "default");
    let mut prediction_free_assessment = assessment;
    prediction_free_assessment.outcome_predictions.clear();
    let without_prediction = with_axis.derive(&prediction_free_assessment, "default");

    assert_reckoning_well_formed(&with_prediction, "axis world final reckoning");
    assert_reckoning_well_formed(&without_prediction, "prediction-free derivation");
    ...
    assert!(
        ...
        "the comparison clone should have no outcome predictions",
    );

    assert_risk_basis_near(
        &with_prediction.assessment.risk,
        &without_prediction.assessment.risk,
        0.0,
        "RiskBasis before and after removing only outcome predictions",
    );
    assert_tag_profile_near(
        &with_prediction.profile.tags,
        &without_prediction.profile.tags,
        0.0,
        "tag profile before and after removing only outcome predictions",
    );
