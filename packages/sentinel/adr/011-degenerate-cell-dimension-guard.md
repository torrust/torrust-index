...
- **Complete ancestor chains for admitted targets.** Competitive candidates are filtered by suffix width before selection. Every ancestor of an admitted target is shallower and therefore at least as wide, so ancestor closure cannot encounter a degenerate node or leave a reporting gap. `degenerate_cells_skipped` measures rejected candidates without changing the complete chain of any target the selector admits.
