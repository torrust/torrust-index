...
**MSRV:** 1.90 — inherited from the workspace `rust-version` and tested in CI.
...
| Accessors (13)                     | `node_count`, `terminal_count`, `semi_internal_count` (𝒪(G_max) arena scan, where G_max = the arena's slot length: the high-water mark of the live G-node count), `structural_mutation_counts`, `budget`, `total_sum`, `config`, `g_root`, `depth_evict`, `depth_create`, `depth_buffer`, `headroom`, `soft_limit` |
